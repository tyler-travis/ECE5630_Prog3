...
\begin{homeworkProblem}
  ...
  \begin{homeworkSection}{(a)}
    What are the number of multiplies and adds required to perform a 1296-point DFT?
    What about a radix-6 FFT?
    \begin{center}
      \noindent\rule{6.5in}{0.4pt}
    \end{center}
  \end{homeworkSection}
  ...
  \begin{homeworkSection}{(b)}
    Verify that your FFT works as expected by computing
    the FFT of 1296 points of a signal created by adding together
    sinusoids of frequencies at $f=17.01Hz$, $f=297.71Hz$, 
    $f=425.35Hz$, and $f=2637Hz$. Use a sample rate of 11.025kHz to
    create the test sinusoids.
  ...
  \begin{homeworkSection}{(c)}
     Plot the magnitude of the FFT output. Which bins have
     values larger than the others? (Remember that there may be some
     computation noise in each bin.)
    \begin{center}
      \noindent\rule{6.5in}{0.4pt}
  ...
  \end{homeworkSection}
  \begin{figure}[h]
      \caption{radix-6 FFT output of the 4 sinusoids}
      \centering
      \includegraphics[width=0.5\textwidth]{plots/Part2-1296-FFT-Mag}
  \end{figure}
\end{homeworkProblem}
\clearpage
...
\begin{homeworkProblem}
  ...
  \begin{center}
    \noindent\rule{6.5in}{0.4pt}
  \end{center}
  The program removes the high frequency components effectively and the output
  sounds as expected with the flute noises no longer being there.
  
  \begin{figure}[h]
      \caption{radix-6 FFT output of the sound file}
      \centering
      \includegraphics[width=0.5\textwidth]{plots/Part3-1296-FFT-Mag-and-Time}
    \end{figure}
    
    Listing \ref{src/main3} shows the first program.

  \cppscript{src/main3}{Program 1 - main3.cpp}
